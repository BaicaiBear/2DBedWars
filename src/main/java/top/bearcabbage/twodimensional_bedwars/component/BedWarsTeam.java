...
public class BedWarsTeam implements ITeam {
    ...
    private void applyTeamUpgrades(net.minecraft.server.world.ServerWorld world) {
        net.minecraft.registry.Registry<net.minecraft.enchantment.Enchantment> registry = world.getRegistryManager()
                .getOptional(net.minecraft.registry.RegistryKeys.ENCHANTMENT).orElseThrow();
        net.minecraft.registry.entry.RegistryEntry<net.minecraft.enchantment.Enchantment> sharpness = registry
                .getEntry(net.minecraft.enchantment.Enchantments.SHARPNESS.getValue()).orElseThrow();
        net.minecraft.registry.entry.RegistryEntry<net.minecraft.enchantment.Enchantment> protection = registry
                .getEntry(net.minecraft.enchantment.Enchantments.PROTECTION.getValue()).orElseThrow();

        for (BedWarsPlayer bwPlayer : players) {
            net.minecraft.server.network.ServerPlayerEntity player = world.getServer().getPlayerManager()
                    .getPlayer(bwPlayer.getUuid());
            if (player != null && bwPlayer.getState() == 1) { // Alive
                ...
                if (hasteLevel > 0) {
                    ...
                    player.addStatusEffect(new StatusEffectInstance(net.minecraft.entity.effect.StatusEffects.HASTE,
                ...
                if (protectionLevel > 0) {
                    for (int i = 0; i < 4; i++) {
                        net.minecraft.item.ItemStack stack = player.getInventory().getStack(36 + i); // 36-39 are armor
                                                                                                     // slots
                        if (!stack.isEmpty()) {
    ...
    private void applyEnchantment(net.minecraft.item.ItemStack stack,
            net.minecraft.registry.entry.RegistryEntry<net.minecraft.enchantment.Enchantment> enchant, int level) {
        net.minecraft.component.type.ItemEnchantmentsComponent enchants = net.minecraft.enchantment.EnchantmentHelper
                .getEnchantments(stack);
        int current = enchants.getLevel(enchant);
        if (current < level) {
            net.minecraft.component.type.ItemEnchantmentsComponent.Builder builder = new net.minecraft.component.type.ItemEnchantmentsComponent.Builder(
                    enchants);
    ...
    private void updateGenerators() {
        ...
        for (BlockPos pos : generators) {
            ...
            for (top.bearcabbage.twodimensional_bedwars.config.GameConfig.ForgeResource res : resources) {
                ...
                net.minecraft.util.Identifier id = net.minecraft.util.Identifier.tryParse(material);
                if (id == null)
                    id = net.minecraft.util.Identifier.of("minecraft", "air");
    ...
    public static BedWarsTeam createTeam(net.minecraft.server.world.ServerWorld world, int id, String name,
            int colorValue,
    ...
    private static net.minecraft.block.Block getBedBlock(String name) {
        if ("Red".equalsIgnoreCase(name))
            return net.minecraft.block.Blocks.RED_BED;
        if ("Blue".equalsIgnoreCase(name))
            return net.minecraft.block.Blocks.BLUE_BED;
        if ("Green".equalsIgnoreCase(name))
            return net.minecraft.block.Blocks.GREEN_BED;
        if ("Yellow".equalsIgnoreCase(name))
            return net.minecraft.block.Blocks.YELLOW_BED;
