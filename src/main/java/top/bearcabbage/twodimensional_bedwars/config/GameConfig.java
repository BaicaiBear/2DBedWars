...
public class GameConfig {
    ...
    public List<BaseGeneratorRecipe> baseGeneratorRecipes = defaultBaseGeneratorRecipes();
    ...
    private static List<BaseGeneratorRecipe> defaultBaseGeneratorRecipes() {
        List<BaseGeneratorRecipe> list = new ArrayList<>();
        list.add(new BaseGeneratorRecipe(0, "minecraft:iron_ingot", 1.0, 1, 48));
        list.add(new BaseGeneratorRecipe(1, "minecraft:iron_ingot", 0.7, 1, 48));
        list.add(new BaseGeneratorRecipe(1, "minecraft:gold_ingot", 4.0, 1, 12));
        list.add(new BaseGeneratorRecipe(2, "minecraft:iron_ingot", 0.5, 1, 64));
        list.add(new BaseGeneratorRecipe(2, "minecraft:gold_ingot", 3.0, 1, 16));
        list.add(new BaseGeneratorRecipe(3, "minecraft:emerald", 10.0, 1, 2));
        return list;
    }

    private static List<Offset> defaultDiamondOffsets() {
        List<Offset> list = new ArrayList<>();
        list.add(new Offset(0, 0, 20));
        list.add(new Offset(0, 0, -20));
        return list;
    }
    ...
    private static List<Offset> defaultEmeraldOffsets() {
        List<Offset> list = new ArrayList<>();
        list.add(new Offset(0, 0, 0));
        return list;
    }

    private static List<Offset> defaultGoldOffsets() {
        List<Offset> list = new ArrayList<>();
        list.add(new Offset(10, 0, 10));
        return list;
    }
    ...
    public static class RestoreConfig {
        ...
        public MapRegion arena2Template = new MapRegion(0, 85, 0, 100, 45, 100);
    }
    ...
    public static void load() {
        ...
        try (FileReader reader = new FileReader(file)) {
            ...
            e.printStackTrace();
            if (INSTANCE == null) INSTANCE = new GameConfig(); // Keep memory safe
        }
    }

    private void ensureDefaults() {
        if (arena1 == null) arena1 = new ArenaDetail("Iron");
        if (arena2 == null) arena2 = new ArenaDetail("Quartz");
        if (baseOffsets == null) baseOffsets = new BaseOffsets();
        if (arenaRestoreConfig == null) arenaRestoreConfig = new RestoreConfig();
        
        if (ironGenerator == null) ironGenerator = new GeneratorSetting(1, 0.5, 48);
        if (goldGenerator == null) goldGenerator = new GeneratorSetting(1, 1.0, 16);
        if (diamondGenerator == null) diamondGenerator = new GeneratorSetting(1, 1.0, 16);
        if (emeraldGenerator == null) emeraldGenerator = new GeneratorSetting(1, 10.0, 5);
        if (quartzGenerator == null) quartzGenerator = new GeneratorSetting(1, 0.5, 64);
        if (netheriteGenerator == null) netheriteGenerator = new GeneratorSetting(1, 10.0, 5);
        
        if (arena1DiamondGenerators == null) arena1DiamondGenerators = defaultDiamondOffsets();
        if (arena1EmeraldGenerators == null) arena1EmeraldGenerators = defaultEmeraldOffsets();
        if (arena2GoldGenerators == null) arena2GoldGenerators = defaultGoldOffsets();
        if (arena2NetheriteGenerators == null) arena2NetheriteGenerators = defaultNetheriteOffsets();
        
        if (ironShop == null) ironShop = defaultIronShop();
        if (goldShop == null) goldShop = defaultGoldShop();
        
        if (baseGeneratorRecipes == null) baseGeneratorRecipes = defaultBaseGeneratorRecipes();
    }
